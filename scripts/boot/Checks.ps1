...
Function Exit-DiscoFun-Updater {
  $caption = ""
  $description = ""
...
Write-Host "[DiscoFun]: $($locales.checking_system)"

Set-Location ..
Set-Location ..

If (-Not (Test-Path ".git")) {
  Write-Host "[DiscoFun]: $($locales.check_failure)"
  Write-Host

  Write-Host "[DiscoFun]: $($locales.check_installation)"
  Write-Host "[DiscoFun]: $($locales.check_repo_download_method)"
  Write-Host "[DiscoFun]: $($locales.get_help) https://discord.gg/9h822H3"

  Exit-DiscoFun-Updater
}

Clear-Host

Write-Host "[DiscoFun]: $($locales.check_success)"
Write-Host

Write-Host "[DiscoFun]: $($locales.updating)"
Write-Host

git pull
If (-Not ($?)) {
  Write-Host "[DiscoFun]: $($locales.updating_failure)"
  Write-Host "[DiscoFun]: $($locales.get_help) https://discord.gg/9h822H3"
  Write-Host
...
If (-Not (Test-Path "node_modules")) {
  Write-Host "[DiscoFun]: Installing dependicies."
  Write-Host
