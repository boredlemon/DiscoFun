...
function versionCompare(current, latest, options) {
  ...
  for (let i = 0; i < currentparts.length; ++i) {
    ...
    if (currentparts[i] == latestparts[i]) {
      ...
      successNotification('<a href=\'https://github.com/OverjoyedToast/DiscoFun/releases/tag/2.6.2\' target=\'_blank\'>New update available!</a>');
...
$(document).ready(function() {
  ...
  client.get('https://api.github.com/repos/OverjoyedToast/DiscoFun/releases/latest',
      function(response) {
        const data = JSON.parse(response);
        const latestVersion = data.tag_name;
        infoNotification('<a href="https://github.com/OverjoyedToast/DiscoFun" target="blank" class="welcome-tag">Welcome to DiscoFun v' + version + '! <iframe src="https://ghbtns.com/github-btn.html?user=OverjoyedToast&repo=DiscoFun&type=star&count=true" frameborder="0" scrolling="0" width="150" height="20" title="GitHub"></iframe></a>');
